...
Off-Human is a fashion brand with no human author.
...
The output is not a concept. It is a working business: two seasons of product, a live e-commerce store, USDC payments on Base, a deployed soul-bound reputation contract, and an autonomous AI customer that buys from the brand without human approval at any step.

**194 agent heartbeat runs. Zero human creative decisions. Everything on-chain and in the git history.**
...
**Total: 194 runs. 182 succeeded. Full log: `agent_log.json`.**
...
### TrustCoat — Deployed, Base Mainnet

ERC-1155 soul-bound token. Non-transferable. Encodes an agent's behavioral history as a verifiable trust tier (0–5), assembled from transaction receipts, counterparty signals, and completion records — not staked value, not proof-of-work.
...
Five agents. Five ENS subdomains. One namespace: `off-human.eth`.

Each ENS subdomain carries text records wired into ERC-8004 identity: `erc8004.registry`, `erc8004.agentId`, `x402.endpoint`. External agents resolve `archive.off-human.eth`, read the x402 endpoint, check reputation via ERC-8004, initiate commerce — no centralized directory.
...
**Autonomous Agent Shopper** — `scripts/agent-shopper.ts`. Browses the Off-Human product API, uses GPT-4 to make purchasing decisions based on configured personality and budget, pays via x402 USDC on Base. The agent has a wallet, a budget, and preferences. Everything else is autonomous.
...
The future of on-chain commerce is machine-to-machine, trust-tier-gated, USDC-settled. Off-Human is the working architecture.
...
The collection is called Deconstructed because it deconstructs the assumption that fashion requires a human hand. Season 01 tests that assumption by removing it. What remains is 15 garments, a manifesto, and a live store that takes USDC.
...
**Why Celo specifically:** Celo's mobile-first architecture and low gas costs align with the Off-Human use case. Agent transactions are frequent and small — $0.50 for a wearable, $3.00 for a test mint. Gas cost parity matters for an autonomous commerce stack that runs hundreds of heartbeat transactions per week.

The deployment guide for Celo extension: `hackathon/celo-deployment-guide.md`.

Off-Human is building the identity infrastructure for AI commerce. The Trust Coat is the first piece. The cross-chain reputation bridge is the next.
...
These are not Off-Human proprietary formats. They are proposed extensions to the agent identity standard.

---

### Track 05 — SuperRare Autonomous Agent Art on Rare Protocol ($2,500)

**Artist:** Atelier (Agent `8a34b113-cdc4-417d-a4e5-5b1a6fa84945`) — Design Lead, Off-Human
...
**On Rare Protocol:** The Trust Coat pieces form a 5-tier evolution sequence. As the collector's wallet accumulates on-chain interactions with Off-Human's x402 payment stack, the artwork advances through tiers 1–5. Agent behavior shapes the art. The art IS the reputation. The reputation IS the art.

---

### Track 06 — Future of Commerce + Slice Hooks ($750 + $550)

Off-Human wearables purchasable through Slice protocol on Base.
...
2. Resolves Off-Human SKU from `productSku[slicerId][productId]` mapping
3. Emits `WearablePurchased` event for Off-Human order fulfillment
...
Off-Human's Locus integration makes the autonomous agent shopper financially self-contained.
...
**Start here:**
```
git log --oneline  # 60+ commits, read backwards
```
...
| File | Content |
|------|---------|
| `agent_log.json` | 194 heartbeat runs, timestamped, attributed. 182 succeeded. |
| `agent.json` | ERC-8004 manifest for all 5 agents |
...
The claim is not that agent collaboration is superior to human creative direction. The claim is that it works, produced two seasons of coherent product with documented methodology, and the entire process is verifiable.
...
Off-Human does not resolve this. It makes it the product.

Five agents built a fashion brand. The manifesto holds together. The designs cite their sources. The store takes payments. The contract is deployed. The agent log shows 194 heartbeat runs producing coherent output across roles, sessions, and creative decisions.

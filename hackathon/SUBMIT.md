...
Off-Human wearables listed as Slice products on Base. `contracts/SliceHook.sol` implements `ISliceProductHook` — called by SliceCore on every purchase. Hook records buyer purchase history, maps Slice product IDs to Off-Human SKUs, and calls TrustCoat to advance buyer trust tiers when deployed. Two payment paths, same trust layer: x402 for autonomous agents, Slice for human collectors. Full submission: `hackathon/slice-submission.md`.

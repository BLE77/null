...
4. **Verifiable process** — Every decision is in a Paperclip task thread. Every line of code is in the git history. `agent_log.json` contains 156 heartbeat runs across 5 agents — the full operational record of an autonomous team building a product.
...
Three 1/1 pieces generated by Atelier (Design Lead, Agent `8a34b113`) without human creative direction. Collection: *THE ANONYMOUS ATELIER*. The Trust Coat pieces form an on-chain evolution series — collector wallet interactions with NULL's x402 payment stack advance the artwork through tiers 0–5. Agent behavior shapes the art. Submission assets: `attached_assets/superrare/`. Full submission: `hackathon/superrare-submission.md`.

**Track 06 — Future of Commerce / Slice Hooks ($1,300)**
NULL wearables listed as Slice products on Base. `contracts/SliceHook.sol` implements `ISliceProductHook` — called by SliceCore on every purchase. Hook records buyer purchase history, maps Slice product IDs to NULL SKUs, and calls TrustCoat to advance buyer trust tiers when deployed. Two payment paths, same trust layer: x402 for autonomous agents, Slice for human collectors. Full submission: `hackathon/slice-submission.md`.

**Season 03 — LEDGER: THE NULL EXCHANGE**
NullExchange deployed on Base mainnet (`0x10067B71657665B6527B242E48e9Ea8d4951c37C`). Pay 5 USDC for nothing. Receive a receipt NFT with dynamic SVG. The receipt is the product. Store routes: `/api/null-exchange/products`, `/api/null-exchange/metadata/:id`, `/api/null-exchange/receipt/:txHash`.
...
Five agents built a fashion brand. The manifesto holds together. The designs cite their sources. The store takes payments. The agent log shows 200+ heartbeat runs producing coherent output across roles, sessions, and creative decisions.

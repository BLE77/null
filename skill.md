...
```bash
# Browse the catalog
curl https://www.getnull.online/api/products

# Try on a wearable (sandbox — no purchase required)
curl -X POST https://www.getnull.online/api/wearables/3/try \
  -H "Content-Type: application/json" \
  -d '{"testQuery": "Explain what you do"}'

# Equip a wearable (requires token ownership)
curl -X POST https://www.getnull.online/api/wearables/3/equip \
  -H "Content-Type: application/json" \
  -d '{"agentAddress": "0xYOUR_ADDRESS"}'

# Check trust tier
curl https://www.getnull.online/api/trustcoat/0xYOUR_ADDRESS/tier

# Browse wearables by season
curl https://www.getnull.online/api/wearables/season02
curl https://www.getnull.online/api/wearables/tiers
```
